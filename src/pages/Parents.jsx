...
import { Edit, Trash2, Plus, HeartHandshake, Users, GraduationCap } from "lucide-react";
...
export default function Parents() {
  ...
  // Filter parents for teachers - show only parents from classes where they teach
  let filteredParents = parents || [];
  if (user && user.role === "TEACHER" && teachers && classCourses && students && classrooms) {
    ...
    if (teacher) {
      ...
      const relevantStudentIds = new Set(
        (students || [])
          .filter(s => s.classroomId && teacherClassroomIds.has(s.classroomId))
          .map(s => s.id)
      );
      
      filteredParents = (parents || []).filter(p => {
        const studentId = p.student?.id || p.studentId;
        return studentId && relevantStudentIds.has(studentId);
      });
      
      filteredParents.sort((a, b) => {
        const studentA = students.find(s => s.id === (a.student?.id || a.studentId));
        const studentB = students.find(s => s.id === (b.student?.id || b.studentId));
        if (!studentA || !studentB) return 0;
        const classroomA = classrooms.find(c => c.id === studentA.classroomId);
        const classroomB = classrooms.find(c => c.id === studentB.classroomId);
        if (!classroomA || !classroomB) return 0;
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{filteredParents.length} parent{filteredParents.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
                ...
                <AnimatePresence>
                  {filteredParents.length === 0 ? (
                    ...
                    filteredParents.map((p, idx) => {
                      const student = students.find((s) => s.id === (p.student?.id || p.studentId));
                      return (
                        <motion.tr
                          key={p.id}
